...
public interface CacheStrategy<Key> {
	/**
	 * Allows the strategy to read the storage state on reinitialization. Needed
	 * mostly for DiskStorage.
	 */
	<T> void initialize(StorageStrategy<Key, T> storage);

	/**
	 * Notifies the strategy that a new key was needed and returns the old key
	 * that occupied the slot (null, same key, otherDifferentKey).
	 * 
	 * @param key
	 * @return key if the update is a hit
	 * @return null if the update is a miss and no key replacement is necessary
	 * @return otherDifferentKey if the update is a miss returns the otherKey
	 *         that needs to be replaced by key
	 */
	Key update(Key key);
}
